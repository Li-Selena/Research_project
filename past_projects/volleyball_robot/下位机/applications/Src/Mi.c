...
void MI_motor_CanTx(MI_Motor_s* hmotor)
{
    ...
    /*检测可用的发送邮箱*/
    if (HAL_CAN_GetTxMailboxesFreeLevel(hmotor->phcan) == 0U) {
        ++mi_tx_dropped;
        return;
    }
    /* 将发送信息添加到发送邮箱中 */
    uint32_t mailbox;
    if (HAL_CAN_AddTxMessage(hmotor->phcan, &CAN_TxHeader_MI, hmotor->txdata, &mailbox) != HAL_OK)
        ++mi_tx_dropped;
}
